export const Robots = [
    {   
        'id': 1,
        'robot_name': 'ETH Bot 1',
        'coin_name': 'ETH',
        'coin_class': 'coin_icon_eth',
        'exchange': 'OKEX',
        'initial_capital': {
            'current': '13.87365489',
            'old': '13.87365489'
        },
        'robot_balance': {
            'current': '13.87365489',
            'old': '13.87365489'
        },
        'performance_val': {
            'current': '+ 3,90%',
            'old': '+ 3,90%'
        },
        'performance_class': 'green_info',
        'started': '18/17/2019',
        'started_day': '24 Days Active',
        'button_val': 'Stop',
        'button_class': '',
        'button_type': '',
        'button_checkbox': 'checked',
        'data': [{
            label: 'ETHBot1Data',
            values: [
                {x: 0, y: 8},
                {x: 1.3, y: 6},
                {x: 3, y: 6.5},
                {x: 3.5, y: 5},
                {x: 4, y: 6},
                {x: 4.5, y: 7},
                {x: 5, y: 7.5},
                {x: 5.5, y: 8}
            ]
        }],
        'statistic': [
            {
                'id': 1,
                'name': 'Average Bars Held',
                'trades': { 'all': 10, 'long': 10, 'short': 10, },
                'state': 'profit'
            },
            {
                'id': 2,
                'name': 'Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'profit'
            },
            {
                'id': 3,
                'name': 'Number of Trades',
                'trades': { 'all': 63, 'long': 37, 'short': 26, },
                'state': 'profit'
            },
            {
                'id': 4,
                'name': 'Average Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'profit'
            },
            {
                'id': 5,
                'name': 'Average Bars Held',
                'trades': { 'all': 10, 'long': 10, 'short': 10, },
                'state': 'profit'
            },
            {
                'id': 6,
                'name': 'Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'profit'
            },
            {
                'id': 7,
                'name': 'Number of Trades',
                'trades': { 'all': 63, 'long': 37, 'short': 26, },
                'state': 'profit'
            },
            {
                'id': 8,
                'name': 'Average Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'profit'
            },
            {
                'id': 8,
                'name': 'Average Bars Held',
                'trades': { 'all': 10, 'long': 10, 'short': 10, },
                'state': 'winner'
            },
            {
                'id': 9,
                'name': 'Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'winner'
            },
            {
                'id': 10,
                'name': 'Number of Trades',
                'trades': { 'all': 63, 'long': 37, 'short': 26, },
                'state': 'winner'
            },
            {
                'id': 11,
                'name': 'Average Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'winner'
            },
            {
                'id': 12,
                'name': 'Average Bars Held',
                'trades': { 'all': 10, 'long': 10, 'short': 10, },
                'state': 'lost'
            },
            {
                'id': 13,
                'name': 'Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'lost'
            },
            {
                'id': 14,
                'name': 'Number of Trades',
                'trades': { 'all': 63, 'long': 37, 'short': 26, },
                'state': 'lost'
            },
            {   
                'id': 15,
                'name': 'Average Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'lost'
            },
        ]
    },
    {   
        'id': 2,
        'robot_name': 'RIPPLE Bot 1',
        'coin_name': 'XRP',
        'coin_class': 'coin_icon_RIPPLE',
        'exchange': 'BITFINEX',
        'initial_capital': {
            'current': '13.87365489',
            'old': '13.87365489'
        },
        'robot_balance': {
            'current': '13.87365489',
            'old': '13.87365489'
        },
        'performance_val': {
            'current': '+ 3,90%',
            'old': '+ 3,90%'
        },
        'performance_class': 'green_info',
        'started': '18/17/2019',
        'started_day': '24 Days Active',
        'button_val': 'Stop',
        'button_class': '',
        'button_type': '',
        'button_checkbox': '',
        'data': [{
            label: 'RIPPLEBot1Data',
            values: [
                {x: 0, y: 8},
                {x: 1.3, y: 7},
                {x: 3, y: 6},
                {x: 3.5, y: 5},
                {x: 4, y: 6},
                {x: 4.5, y: 6},
                {x: 5, y: 7},
                {x: 5.5, y: 8}
            ]
        }],
        'statistic': [
            {
                'id': 1,
                'name': 'Average Bars Held',
                'trades': { 'all': 10, 'long': 10, 'short': 10, },
                'state': 'profit'
            },
            {
                'id': 2,
                'name': 'Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'profit'
            },
            {
                'id': 3,
                'name': 'Number of Trades',
                'trades': { 'all': 63, 'long': 37, 'short': 26, },
                'state': 'profit'
            },
            {
                'id': 4,
                'name': 'Average Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'profit'
            },
            {
                'id': 5,
                'name': 'Average Bars Held',
                'trades': { 'all': 10, 'long': 10, 'short': 10, },
                'state': 'profit'
            },
            {
                'id': 6,
                'name': 'Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'profit'
            },
            {
                'id': 7,
                'name': 'Number of Trades',
                'trades': { 'all': 63, 'long': 37, 'short': 26, },
                'state': 'profit'
            },
            {
                'id': 8,
                'name': 'Average Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'profit'
            },
            {
                'id': 8,
                'name': 'Average Bars Held',
                'trades': { 'all': 10, 'long': 10, 'short': 10, },
                'state': 'winner'
            },
            {
                'id': 9,
                'name': 'Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'winner'
            },
            {
                'id': 10,
                'name': 'Number of Trades',
                'trades': { 'all': 63, 'long': 37, 'short': 26, },
                'state': 'winner'
            },
            {
                'id': 11,
                'name': 'Average Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'winner'
            },
            {
                'id': 12,
                'name': 'Average Bars Held',
                'trades': { 'all': 10, 'long': 10, 'short': 10, },
                'state': 'lost'
            },
            {
                'id': 13,
                'name': 'Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'lost'
            },
            {
                'id': 14,
                'name': 'Number of Trades',
                'trades': { 'all': 63, 'long': 37, 'short': 26, },
                'state': 'lost'
            },
            {   
                'id': 15,
                'name': 'Average Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'lost'
            },
        ]
    },
    {   
        'id': 3,
        'robot_name': 'ETH Bot 2',
        'coin_name': 'ETH',
        'coin_class': 'coin_icon_eth',
        'exchange': 'OKEX',
        'initial_capital': {
            'current': '13.87365489',
            'old': '13.87365489'
        },
        'robot_balance': {
            'current': '13.87365489',
            'old': '13.87365489'
        },
        'performance_val': {
            'current': '- 3,90%',
            'old': '+ 1,90%'
        },
        'performance_class': 'green_info',
        'started': '18/17/2019',
        'started_day': '24 Days Active',
        'button_val': 'Resume',
        'button_class': 'button_resume',
        'button_type': '',
        'button_checkbox': '',
        'data': [{
            label: 'ETHBot2Data',
            values: [
                {x: 0, y: 8},
                {x: 1.3, y: 5},
                {x: 3, y: 6},
                {x: 3.5, y: 6.5},
                {x: 4, y: 6},
                {x: 4.5, y: 6},
                {x: 5, y: 7},
                {x: 5.5, y: 8}
            ]
        }],
        'statistic': [
            {
                'id': 1,
                'name': 'Average Bars Held',
                'trades': { 'all': 10, 'long': 10, 'short': 10, },
                'state': 'profit'
            },
            {
                'id': 2,
                'name': 'Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'profit'
            },
            {
                'id': 3,
                'name': 'Number of Trades',
                'trades': { 'all': 63, 'long': 37, 'short': 26, },
                'state': 'profit'
            },
            {
                'id': 4,
                'name': 'Average Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'profit'
            },
            {
                'id': 5,
                'name': 'Average Bars Held',
                'trades': { 'all': 10, 'long': 10, 'short': 10, },
                'state': 'profit'
            },
            {
                'id': 6,
                'name': 'Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'profit'
            },
            {
                'id': 7,
                'name': 'Number of Trades',
                'trades': { 'all': 63, 'long': 37, 'short': 26, },
                'state': 'profit'
            },
            {
                'id': 8,
                'name': 'Average Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'profit'
            },
            {
                'id': 8,
                'name': 'Average Bars Held',
                'trades': { 'all': 10, 'long': 10, 'short': 10, },
                'state': 'winner'
            },
            {
                'id': 9,
                'name': 'Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'winner'
            },
            {
                'id': 10,
                'name': 'Number of Trades',
                'trades': { 'all': 63, 'long': 37, 'short': 26, },
                'state': 'winner'
            },
            {
                'id': 11,
                'name': 'Average Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'winner'
            },
            {
                'id': 12,
                'name': 'Average Bars Held',
                'trades': { 'all': 10, 'long': 10, 'short': 10, },
                'state': 'lost'
            },
            {
                'id': 13,
                'name': 'Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'lost'
            },
            {
                'id': 14,
                'name': 'Number of Trades',
                'trades': { 'all': 63, 'long': 37, 'short': 26, },
                'state': 'lost'
            },
            {   
                'id': 15,
                'name': 'Average Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'lost'
            },
        ]
    },
    {   
        'id': 4,
        'robot_name': 'BTC Bot 1',
        'coin_name': 'BTC',
        'coin_class': 'coin_icon_bitcoin',
        'exchange': 'OKEX',
        'initial_capital': {
            'current': '13.87365489',
            'old': '13.87365489'
        },
        'robot_balance': {
            'current': '13.87365489',
            'old': '13.87365489'
        },
        'performance_val': {
            'current': '+ 3,90%',
            'old': '+ 3,90%'
        },
        'performance_class': 'green_info',
        'started': '18/17/2019',
        'started_day': '24 Days Active',
        'button_val': 'Resume',
        'button_class': 'button_resume',
        'button_type': '',
        'button_checkbox': 'checked',
        'data': [{
            label: 'BTCBot1Data',
            values: [
                {x: 0, y: 8},
                {x: 1.3, y: 4},
                {x: 3, y: 2},
                {x: 3.5, y: 3.5},
                {x: 4, y: 8},
                {x: 4.5, y: 6.5},
                {x: 5, y: 5},
                {x: 5.5, y: 4}
            ]
        }],
        'statistic': [
            {
                'id': 1,
                'name': 'Average Bars Held',
                'trades': { 'all': 10, 'long': 10, 'short': 10, },
                'state': 'profit'
            },
            {
                'id': 2,
                'name': 'Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'profit'
            },
            {
                'id': 3,
                'name': 'Number of Trades',
                'trades': { 'all': 63, 'long': 37, 'short': 26, },
                'state': 'profit'
            },
            {
                'id': 4,
                'name': 'Average Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'profit'
            },
            {
                'id': 5,
                'name': 'Average Bars Held',
                'trades': { 'all': 10, 'long': 10, 'short': 10, },
                'state': 'profit'
            },
            {
                'id': 6,
                'name': 'Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'profit'
            },
            {
                'id': 7,
                'name': 'Number of Trades',
                'trades': { 'all': 63, 'long': 37, 'short': 26, },
                'state': 'profit'
            },
            {
                'id': 8,
                'name': 'Average Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'profit'
            },
            {
                'id': 8,
                'name': 'Average Bars Held',
                'trades': { 'all': 10, 'long': 10, 'short': 10, },
                'state': 'winner'
            },
            {
                'id': 9,
                'name': 'Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'winner'
            },
            {
                'id': 10,
                'name': 'Number of Trades',
                'trades': { 'all': 63, 'long': 37, 'short': 26, },
                'state': 'winner'
            },
            {
                'id': 11,
                'name': 'Average Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'winner'
            },
            {
                'id': 12,
                'name': 'Average Bars Held',
                'trades': { 'all': 10, 'long': 10, 'short': 10, },
                'state': 'lost'
            },
            {
                'id': 13,
                'name': 'Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'lost'
            },
            {
                'id': 14,
                'name': 'Number of Trades',
                'trades': { 'all': 63, 'long': 37, 'short': 26, },
                'state': 'lost'
            },
            {   
                'id': 15,
                'name': 'Average Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'lost'
            },
        ]
    },
    {   
        'id': 5,
        'robot_name': 'ETH Bot 3',
        'coin_name': 'ETH',
        'coin_class': 'coin_icon_eth',
        'exchange': 'OKEX',
        'initial_capital': {
            'current': '13.87365489',
            'old': '13.87365489'
        },
        'robot_balance': {
            'current': '13.87365489',
            'old': '13.87365489'
        },
        'performance_val': {
            'current': '+ 3,90%',
            'old': '+ 3,90%'
        },
        'performance_class': 'green_info',
        'started': '18/17/2019',
        'started_day': '24 Days Active',
        'button_val': 'Run',
        'button_class': 'button_run',
        'button_type': '',
        'button_checkbox': 'checked',
        'data': [{
            label: 'ETHBot3Data',
            values: [
                {x: 0, y: 8},
                {x: 1.3, y: 5},
                {x: 3, y: 6},
                {x: 3.5, y: 5},
                {x: 4, y: 6},
                {x: 4.5, y: 8},
                {x: 5, y: 7},
                {x: 5.5, y: 5}
            ]
        }],
        'statistic': [
            {
                'id': 1,
                'name': 'Average Bars Held',
                'trades': { 'all': 10, 'long': 10, 'short': 10, },
                'state': 'profit'
            },
            {
                'id': 2,
                'name': 'Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'profit'
            },
            {
                'id': 3,
                'name': 'Number of Trades',
                'trades': { 'all': 63, 'long': 37, 'short': 26, },
                'state': 'profit'
            },
            {
                'id': 4,
                'name': 'Average Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'profit'
            },
            {
                'id': 5,
                'name': 'Average Bars Held',
                'trades': { 'all': 10, 'long': 10, 'short': 10, },
                'state': 'profit'
            },
            {
                'id': 6,
                'name': 'Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'profit'
            },
            {
                'id': 7,
                'name': 'Number of Trades',
                'trades': { 'all': 63, 'long': 37, 'short': 26, },
                'state': 'profit'
            },
            {
                'id': 8,
                'name': 'Average Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'profit'
            },
            {
                'id': 8,
                'name': 'Average Bars Held',
                'trades': { 'all': 10, 'long': 10, 'short': 10, },
                'state': 'winner'
            },
            {
                'id': 9,
                'name': 'Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'winner'
            },
            {
                'id': 10,
                'name': 'Number of Trades',
                'trades': { 'all': 63, 'long': 37, 'short': 26, },
                'state': 'winner'
            },
            {
                'id': 11,
                'name': 'Average Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'winner'
            },
            {
                'id': 12,
                'name': 'Average Bars Held',
                'trades': { 'all': 10, 'long': 10, 'short': 10, },
                'state': 'lost'
            },
            {
                'id': 13,
                'name': 'Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'lost'
            },
            {
                'id': 14,
                'name': 'Number of Trades',
                'trades': { 'all': 63, 'long': 37, 'short': 26, },
                'state': 'lost'
            },
            {   
                'id': 15,
                'name': 'Average Net Profit',
                'trades': { 'all': 295.09, 'long': 229.54, 'short': 388.38, },
                'state': 'lost'
            },
        ]
    }
];

export const Crypto = [
    {   
        'id': 1,
        'code': 'BTC',
        'balance': {
            'name': 'Bitcoin',
            'dollar': '$ 6,312.31',
            'icon': 'crypto_balance_icon_bitcoin',
            'current': '3.87235789 BTC'
        },
        'change': {
            'proc': '+ 12,23%',
            'val': '+ 12,23%',
        },
        'market_cap': {
            'dollar': '$ 6,312.31',
            'value': '0.78365479 BTC',
        },
        'volume_24h': {
            'dollar': '$ 6,312.31',
            'value': '0.78365479 BTC',
        },
        'circulating_supply': {
            'dollar': '$ 6,312.31',
            'value': '0.78365479 BTC',
        },
        'max_supply': {
            'dollar': '$ 6,312.31',
            'value': '0.78365479 BTC',
        },
        'news': [
            {
                'id': 1,
                'image': '/image/Bitmap.png',
                'title': 'CNN',
                'date': '19 min ago',
                'text': 'As Bitcoin\'s Slide Continues, Prices Look Towards $8K',
                'content': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion. Slump Begone: What’s Next For Cryptocurrencies? Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December'
            },
            {
                'id': 2,
                'image': '/image/Bitmap1.png',
                'title': 'CRYPTOSTUFF',
                'date': 'Jan 17, 2018',
                'text': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion',
                'content': 'Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December. As Bitcoin\'s Slide Continues, Prices Look Towards $8K. Israel Releases Draft Plan for Taxing ICOs'
            },
            {
                'id': 3,
                'image': '/image/Bitmap2.png',
                'title': 'CNN',
                'date': '19 min ago',
                'text': 'As Bitcoin\'s Slide Continues, Prices Look Towards $8K',
                'content': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion. Slump Begone: What’s Next For Cryptocurrencies? Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December'
            },
            {
                'id': 4,
                'image': '/image/Bitmap3.png',
                'title': 'CRYPTOSTUFF',
                'date': 'Jan 15, 2018',
                'text': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion',
                'content': 'Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December. As Bitcoin\'s Slide Continues, Prices Look Towards $8K. Israel Releases Draft Plan for Taxing ICOs'
            },
            {
                'id': 5,
                'image': '/image/Bitmap4.png',
                'title': 'CNN',
                'date': 'Jan 12, 2018',
                'text': 'As Bitcoin\'s Slide Continues, Prices Look Towards $8K',
                'content': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion. Slump Begone: What’s Next For Cryptocurrencies? Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December'

            }
        ]
    },
    {
        'id': 2,
        'code': 'ETH',
        'balance': {
            'name': 'Etherium',
            'dollar': '$ 1,422.11',
            'icon': 'crypto_balance_icon_etherium',
            'current': '1.98362546 ETH'
        },
        'change': {
            'proc': '- 1,53%',
            'val': '- 40,75 $',
        },
        'market_cap': {
            'dollar': '$ 1,876.14',
            'value': '0.78365479 ETH',
        },
        'volume_24h': {
            'dollar': '$ 1,876.14',
            'value': '0.78365479 ETH',
        },
        'circulating_supply': {
            'dollar': '$ 1,876.14',
            'value': '0.78365479 ETH',
        },
        'max_supply': {
            'dollar': '$ 1,876.14',
            'value': '0.78365479 ETH',
        },
        'news': [
            {
                'id': 1,
                'image': '/image/Bitmap.png',
                'title': 'CNN',
                'date': '19 min ago',
                'text': 'As Bitcoin\'s Slide Continues, Prices Look Towards $8K',
                'content': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion. Slump Begone: What’s Next For Cryptocurrencies? Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December'
            },
            {
                'id': 2,
                'image': '/image/Bitmap1.png',
                'title': 'CRYPTOSTUFF',
                'date': 'Jan 17, 2018',
                'text': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion',
                'content': 'Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December. As Bitcoin\'s Slide Continues, Prices Look Towards $8K. Israel Releases Draft Plan for Taxing ICOs'
            },
            {
                'id': 3,
                'image': '/image/Bitmap2.png',
                'title': 'CNN',
                'date': '19 min ago',
                'text': 'As Bitcoin\'s Slide Continues, Prices Look Towards $8K',
                'content': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion. Slump Begone: What’s Next For Cryptocurrencies? Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December'
            },
            {
                'id': 4,
                'image': '/image/Bitmap3.png',
                'title': 'CRYPTOSTUFF',
                'date': 'Jan 15, 2018',
                'text': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion',
                'content': 'Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December. As Bitcoin\'s Slide Continues, Prices Look Towards $8K. Israel Releases Draft Plan for Taxing ICOs'
            },
            {
                'id': 5,
                'image': '/image/Bitmap4.png',
                'title': 'CNN',
                'date': 'Jan 12, 2018',
                'text': 'As Bitcoin\'s Slide Continues, Prices Look Towards $8K',
                'content': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion. Slump Begone: What’s Next For Cryptocurrencies? Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December'

            }
        ]
    },
    {
        'id': 3,
        'code': 'RPL',
        'balance': {
            'name': 'Ripple',
            'dollar': '$ 37.11',
            'icon': 'crypto_balance_icon_RIPPLE',
            'current': '3.87235789 XRP'
        },
        'change': {
            'proc': '+ 10,16%',
            'val': '+ 40,75 $',
        },
        'market_cap': {
            'dollar': '$ 48,15',
            'value': '3.87235789 XRP',
        },
        'volume_24h': {
            'dollar': '$ 48,15',
            'value': '3.87235789 XRP',
        },
        'circulating_supply': {
            'dollar': '$ 48,15',
            'value': '3.87235789 XRP',
        },
        'max_supply': {
            'dollar': '$ 48,15',
            'value': '3.87235789 XRP',
        },
        'news': [
            {
                'id': 1,
                'image': '/image/Bitmap.png',
                'title': 'CNN',
                'date': '19 min ago',
                'text': 'As Bitcoin\'s Slide Continues, Prices Look Towards $8K',
                'content': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion. Slump Begone: What’s Next For Cryptocurrencies? Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December'
            },
            {
                'id': 2,
                'image': '/image/Bitmap1.png',
                'title': 'CRYPTOSTUFF',
                'date': 'Jan 17, 2018',
                'text': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion',
                'content': 'Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December. As Bitcoin\'s Slide Continues, Prices Look Towards $8K. Israel Releases Draft Plan for Taxing ICOs'
            },
            {
                'id': 3,
                'image': '/image/Bitmap2.png',
                'title': 'CNN',
                'date': '19 min ago',
                'text': 'As Bitcoin\'s Slide Continues, Prices Look Towards $8K',
                'content': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion. Slump Begone: What’s Next For Cryptocurrencies? Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December'
            },
            {
                'id': 4,
                'image': '/image/Bitmap3.png',
                'title': 'CRYPTOSTUFF',
                'date': 'Jan 15, 2018',
                'text': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion',
                'content': 'Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December. As Bitcoin\'s Slide Continues, Prices Look Towards $8K. Israel Releases Draft Plan for Taxing ICOs'
            },
            {
                'id': 5,
                'image': '/image/Bitmap4.png',
                'title': 'CNN',
                'date': 'Jan 12, 2018',
                'text': 'As Bitcoin\'s Slide Continues, Prices Look Towards $8K',
                'content': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion. Slump Begone: What’s Next For Cryptocurrencies? Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December'

            }
        ]
    },
    {
        'id': 4,
        'code': 'LCT',
        'balance': {
            'name': 'Litecoin',
            'dollar': '$ 48.74',
            'icon': 'crypto_balance_icon_litecoin',
            'current': '1.98362546 LTC'
        },
        'change': {
            'proc': '+ 8,78%',
            'val': '+ 40,75 $',
        },
        'market_cap': {
            'dollar': '$ 48,98',
            'value': '0.78365479 LTC',
        },
        'volume_24h': {
            'dollar': '$ 48,98',
            'value': '0.78365479 LTC',
        },
        'circulating_supply': {
            'dollar': '$ 48,98',
            'value': '0.78365479 LTC',
        },
        'max_supply': {
            'dollar': '$ 48,98',
            'value': '0.78365479 LTC',
        },
        'news': [
            {
                'id': 1,
                'image': '/image/Bitmap.png',
                'title': 'CNN',
                'date': '19 min ago',
                'text': 'As Bitcoin\'s Slide Continues, Prices Look Towards $8K',
                'content': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion. Slump Begone: What’s Next For Cryptocurrencies? Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December'
            },
            {
                'id': 2,
                'image': '/image/Bitmap1.png',
                'title': 'CRYPTOSTUFF',
                'date': 'Jan 17, 2018',
                'text': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion',
                'content': 'Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December. As Bitcoin\'s Slide Continues, Prices Look Towards $8K. Israel Releases Draft Plan for Taxing ICOs'
            },
            {
                'id': 3,
                'image': '/image/Bitmap2.png',
                'title': 'CNN',
                'date': '19 min ago',
                'text': 'As Bitcoin\'s Slide Continues, Prices Look Towards $8K',
                'content': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion. Slump Begone: What’s Next For Cryptocurrencies? Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December'
            },
            {
                'id': 4,
                'image': '/image/Bitmap3.png',
                'title': 'CRYPTOSTUFF',
                'date': 'Jan 15, 2018',
                'text': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion',
                'content': 'Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December. As Bitcoin\'s Slide Continues, Prices Look Towards $8K. Israel Releases Draft Plan for Taxing ICOs'
            },
            {
                'id': 5,
                'image': '/image/Bitmap4.png',
                'title': 'CNN',
                'date': 'Jan 12, 2018',
                'text': 'As Bitcoin\'s Slide Continues, Prices Look Towards $8K',
                'content': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion. Slump Begone: What’s Next For Cryptocurrencies? Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December'

            }
        ]
    },
    {
        'id': 5,
        'code': 'DCH',
        'balance': {
            'name': 'Dash',
            'dollar': '$ 48.74',
            'icon': 'crypto_balance_icon_dash',
            'current': '3.87235789 DASH'
        },
        'change': {
            'proc': '+ 8,78%',
            'val': '+ 40,75 $',
        },
        'market_cap': {
            'dollar': '$ 467,18',
            'value': '0.78365479 DASH',
        },
        'volume_24h': {
            'dollar': '$ 467,18',
            'value': '0.78365479 DASH',
        },
        'circulating_supply': {
            'dollar': '$ 467,18',
            'value': '0.78365479 DASH',
        },
        'max_supply': {
            'dollar': '$ 467,18',
            'value': '0.78365479 DASH',
        },
        'news': [
            {
                'id': 1,
                'image': '/image/Bitmap.png',
                'title': 'CNN',
                'date': '19 min ago',
                'text': 'As Bitcoin\'s Slide Continues, Prices Look Towards $8K',
                'content': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion. Slump Begone: What’s Next For Cryptocurrencies? Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December'
            },
            {
                'id': 2,
                'image': '/image/Bitmap1.png',
                'title': 'CRYPTOSTUFF',
                'date': 'Jan 17, 2018',
                'text': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion',
                'content': 'Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December. As Bitcoin\'s Slide Continues, Prices Look Towards $8K. Israel Releases Draft Plan for Taxing ICOs'
            },
            {
                'id': 3,
                'image': '/image/Bitmap2.png',
                'title': 'CNN',
                'date': '19 min ago',
                'text': 'As Bitcoin\'s Slide Continues, Prices Look Towards $8K',
                'content': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion. Slump Begone: What’s Next For Cryptocurrencies? Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December'
            },
            {
                'id': 4,
                'image': '/image/Bitmap3.png',
                'title': 'CRYPTOSTUFF',
                'date': 'Jan 15, 2018',
                'text': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion',
                'content': 'Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December. As Bitcoin\'s Slide Continues, Prices Look Towards $8K. Israel Releases Draft Plan for Taxing ICOs'
            },
            {
                'id': 5,
                'image': '/image/Bitmap4.png',
                'title': 'CNN',
                'date': 'Jan 12, 2018',
                'text': 'As Bitcoin\'s Slide Continues, Prices Look Towards $8K',
                'content': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion. Slump Begone: What’s Next For Cryptocurrencies? Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December'

            }
        ]
    },
    {
        'id': 6,
        'code': 'ETC',
        'balance': {
            'name': 'Eth. Classic',
            'dollar': '$ 789,15',
            'icon': 'crypto_balance_icon_ETH_Classic',
            'current': '3.87235789 ETC'
        },
        'change': {
            'proc': '+ 2,19%',
            'val': '+ 40,75 $',
        },
        'market_cap': {
            'dollar': '$ 765,16',
            'value': '0.78365479 ETC',
        },
        'volume_24h': {
            'dollar': '$ 765,16',
            'value': '0.78365479 ETC',
        },
        'circulating_supply': {
            'dollar': '$ 765,16',
            'value': '0.78365479 ETC',
        },
        'max_supply': {
            'dollar': '$ 765,16',
            'value': '0.78365479 ETC',
        },
        'news': [
            {
                'id': 1,
                'image': '/image/Bitmap.png',
                'title': 'CNN',
                'date': '19 min ago',
                'text': 'As Bitcoin\'s Slide Continues, Prices Look Towards $8K',
                'content': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion. Slump Begone: What’s Next For Cryptocurrencies? Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December'
            },
            {
                'id': 2,
                'image': '/image/Bitmap1.png',
                'title': 'CRYPTOSTUFF',
                'date': 'Jan 17, 2018',
                'text': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion',
                'content': 'Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December. As Bitcoin\'s Slide Continues, Prices Look Towards $8K. Israel Releases Draft Plan for Taxing ICOs'
            },
            {
                'id': 3,
                'image': '/image/Bitmap2.png',
                'title': 'CNN',
                'date': '19 min ago',
                'text': 'As Bitcoin\'s Slide Continues, Prices Look Towards $8K',
                'content': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion. Slump Begone: What’s Next For Cryptocurrencies? Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December'
            },
            {
                'id': 4,
                'image': '/image/Bitmap3.png',
                'title': 'CRYPTOSTUFF',
                'date': 'Jan 15, 2018',
                'text': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion',
                'content': 'Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December. As Bitcoin\'s Slide Continues, Prices Look Towards $8K. Israel Releases Draft Plan for Taxing ICOs'
            },
            {
                'id': 5,
                'image': '/image/Bitmap4.png',
                'title': 'CNN',
                'date': 'Jan 12, 2018',
                'text': 'As Bitcoin\'s Slide Continues, Prices Look Towards $8K',
                'content': 'Bitcoin Price Leads Crypto Comeback after Market Briefly Dips Below $250 Billion. Slump Begone: What’s Next For Cryptocurrencies? Bitcoin\'s Price Drops Below $10,000 for First Time Since Early December'

            }
        ]
    }
];

export const Balance = [
    {
        'name': 'Bitcoin',
        'class': 'col-4 crypto_balance_icon crypto_balance_icon_bitcoin',
        'dollar': '$ 6,422.11',
        'grouth': {
            'percent': '+ 12,23%',
            'dollar': '+ 40,75 $'
        },
        'crypto': '1.23845521 BTC'
    },
    {
        'name': 'Etherium',
        'class': 'col-4 crypto_balance_icon crypto_balance_icon_etherium',
        'dollar': '$ 421,21',
        'grouth': {
            'percent': '− 18,27%',
            'dollar': '− 24,12 $'
        },
        'crypto': '134.28762974 ETH'
    },
    {
        'name': 'Ripple',
        'class': 'col-4 crypto_balance_icon crypto_balance_icon_RIPPLE',
        'dollar': '$ 64,41',
        'grouth': {
            'percent': '+ 8,13%',
            'dollar': '+ 13,89 $'
        },
        'crypto': '160.29867236 XRP'
    },
];

export const Account = {
    'balances': [
        {'name': 'BTC', 'value': '2.23845521 BTC'},
        {'name': 'USD', 'value': '$ 6,537.13'},
        {'name': 'EUR', 'value': '€ 6,537.13'}
    ],
    'accounts': [
        {'name': 'My Bitfinex ETH 001', 'exchange': 'BITFINEX',
         'main_keys_st': 'Verified', 'spared_keys_st': 'None',
         'currency': 'BTC, ETH'
        },
        {'name': 'My Bitfinex ETH 002', 'exchange': 'OKEX',
         'main_keys_st': 'Wrong Keys', 'spared_keys_st': 'Verified',
         'currency': 'BTC'
        },
        {'name': 'My Bitfinex ETH 003', 'exchange': 'BITFINEX',
         'main_keys_st': 'Verified', 'spared_keys_st': 'Wrong Keys',
         'currency': 'ETH'
        },
    ]
};
